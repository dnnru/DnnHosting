...
namespace Italliance.Modules.DnnHosting.Components.ExcelDataExtractor
{
    ...
    {
        internal static Action<TModel, TValue> CreatePropertyValueSetterAction<TModel, TValue>(this Expression<Func<TModel, TValue>> propertyExpression)
        {
            var parameter = Expression.Parameter(typeof(TValue));
            var setPropActionExpression =
                Expression.Lambda<Action<TModel, TValue>>(Expression.Assign(propertyExpression.Body, parameter), propertyExpression.Parameters[0], parameter);

            return setPropActionExpression.Compile();
        }

        internal static void ValidatePropertyExpressionType<TModel, TCollectionItem, TCollectionProperty>(this Expression<Func<TModel, TCollectionProperty>> collectionPropertyExpr)
            where TCollectionProperty : class, ICollection<TCollectionItem>, new()
        {
            ...
            if (!(propertyInfo is null))
            {
                ...
                {
                    throw new
                        ArgumentException($"The property for the given collection expression must be of type {expectedType.FullName}. Given type: {genericCollectionType.FullName}");
                }
            }
            ...
            MemberExpression GetMemberExpression()
            {
                ...
                throw new
                    ArgumentException("The expression property is not of a valid type. Ensure the expression points to a property member in the model i.e. 'm => m.MyProperty'.");
            }
        }
    }
}